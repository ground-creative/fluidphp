	...
	class PtcRouter
	{
		...
		*/
		public static function controller( $route , $controller )
		{
			...
			{
				trigger_error( 'Controller class ' . 
					$controller . ' could not be found!' , E_USER_ERROR );
				return false;
			}
		...
		*/
		protected static function _buildRoutes( $group , $prefix , $routes = array( ) , $checkErrors = false )
		{
			foreach ( $group->routes as $key => $val )
			{
				...
				{
					foreach ( $group->patterns as $prm => $regex )
					{
						$val->where( $prm , $regex );
					}
				}
		...
		*/
		...
		{
			if ( $route->filters ) // check if filters names actually exist
			{
				if ( @$route->filters[ $type ] )
				{
					foreach ( @$route->filters[ $type ] as $filter )
					{
						...
						{
							trigger_error( 'Filter name ' . $filter . 
