	...
	class Resources
	{
		...
		*/
		protected static function _mergeBlocks( $page , $blocks , $type )
		{
			...
			$string = null;
			$a = 0;
			$lang = Website::getLang( 'js' );
			if ( 'js' === $type && \App::option( 'website.use_app_prototype_js' ) )
			{
				$string .= "\t" . '<!-- js app prototype included automatically -->' . "\n";
				$path = Website::getPath( ) . '/js/website-helper/app/App.js';
				$string .= "\t" . '<script type="text/javascript" src="' . $path . '"></script>' . "\n";
				$path = Website::getPath( ) . '/js/website-helper/app/BaseModel.js';
				$string .= "\t" . '<script type="text/javascript" src="' . $path . '"></script>' . "\n";
				$path = Website::getPath( ) . '/js/website-helper/app/BaseController.js';
				$string .= "\t" . '<script type="text/javascript" src="' . $path . '"></script>' . "\n";
				$path = Website::getPath( ) . '/js/website-helper/app/BaseContainer.js';
				$string .= "\t" . '<script type="text/javascript" src="' . $path . '"></script>' . "\n";
				$path = Website::getPath( ) . '/js/website-helper/app/BaseElements.js';
				$string .= "\t" . '<script type="text/javascript" src="' . $path . '"></script>' . "\n";
				$path = Website::getPath( ) . '/js/website-helper/app/BaseForm.js';
				$string .= "\t" . '<script type="text/javascript" src="' . $path . '"></script>' . "\n";
				$path = Website::getPath( ) . '/js/website-helper/app/Validator.js';
				$string .= "\t" . '<script type="text/javascript" src="' . $path . '"></script>' . "\n";
			}
			...
			if ( 'js' === $type && $helpers )
			{
				...
				{
					$path = Website::getPath( ) . '/js/website-helper/app/helpers/' . $helper . '.js';
					$string .= "\t" . '<script type="text/javascript" src="' . $path . '"></script>' . "\n";
				}
			}	
			...
			{
				$path = Website::getPath( ) . '/js/website-helper/lang/' . $lang . '.js';
		...
		*/
		protected static function _build( $items , $type , $comment = null )
		{
			...
			foreach ( $items as $item )
			{
				...
				$path = ( !static::$_resources[ $item ][ 'external' ] ) ? Website::getPath( ) : null;
